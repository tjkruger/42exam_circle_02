...
int find_min(int n1, int n2, int n3)// 2 2 3 as input
{
	int min = n1;// min = 2
...
void find_big_square(t_map* map, t_square* square, t_elements* elements)
{
	...
	for(int i = 0; i < map->height; i++) //setting up map
	{
	...
	for(int i = 0; i < map->height; i++)
	{
		for(int j = 0; j < map->width; j++)
		{
			if(map->grid[i][j] == elements->obstacle) //obstacle check
				matrix[i][j] = 0;
			else if(i == 0 || j == 0)//case for first row or first column always 1 if not obstacle
				matrix[i][j] = 1;
			else {
				int min = find_min(matrix[i - 1][j],matrix[i - 1][j - 1], matrix[i][j - 1]);
				matrix[i][j] = min + 1;
			}//this kinda come like always not left top border and not obstacle
			// visual kinda like this:
			// you map is 10 x 10
			// pos 4  4
			// when i go now in this find min it uses the following values as input args 
			// matrix[i - 1][j] = matrix[4 - 1][4] = matrix[3][4] = 2
			// matrix[i - 1][j - 1] = matrix[4 - 1][4 - 1] = matrix[3][3] = 2
			// matrix[i][j - 1] = matrix[4][4 - 1] = matrix[4][3] = 3
			//then in min
