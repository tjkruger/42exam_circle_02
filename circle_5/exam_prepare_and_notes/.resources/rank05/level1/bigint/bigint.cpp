...
{
	std::string revStr;//is fine because he make a new string reversed
	for(size_t i = str.length(); i > 0; i--) //as long not zero move
	{
...
//////////// bc of itia logic use we need evertime to reverse string in proper repensentation
	...
	//////////// this part is for even out lenght of both string
	int carry = 0;
	int digit1;
	int digit2;
	size_t len = str1.length();
	for(size_t i = 0; i < len; i++)//well hey take now from the str1 the lenght again after maybe pushback stuff from before
	{
		digit1 = str1[i] - '0';
...
bool bigint::operator>(const bigint& other) const
{
	return(!(((*this) < other))); // is shit when both same number :D
}
