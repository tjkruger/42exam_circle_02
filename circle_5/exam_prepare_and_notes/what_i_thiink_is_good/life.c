...
int init_game(t_game *g, char **av) {
	g->w = atoi(av[1]); g->h = atoi(av[2]); g->iter = atoi(av[3]);
	g->pi = 0; g->pj = 0; g->draw = 0;
	g->board = malloc(g->h * sizeof(char *));
	if (!g->board) return -1;
	for (int i = 0; i < g->h; i++) {
		g->board[i] = malloc(g->w);
		if (!g->board[i]) return -1;
		for (int j = 0; j < g->w; j++) g->board[i][j] = ' ';
	}
	return 0;
}
